...
set.seed(123456)	                         
#sample size (number of county x year in neighbor sample)
sample_size = nrow(filter(countysumm, mainsamp == 1 & neighbor_samp == 1))           
#hypothesized treatment effect
effect = 0.1                      
#the proportion of the total sample in the treatment group
prop = nrow(filter(countysumm, TREAT == 1 & YEAR == 1930 & mainsamp == 1))/nrow(filter(countysumm, mainsamp == 1 & neighbor_samp == 1))
#define the significance value. Here we are conducting a two-sided test.
alpha = 0.05                  
#number of simulations
sim.size = 2000            
#the kind of test (two, left or right)
side = "two"                                                                    
