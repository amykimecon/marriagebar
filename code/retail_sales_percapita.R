# summarizing retail sales per capita trends by county & state
...
bytreat <- countysumm_pops %>% filter(neighbor_samp == 1) %>% group_by(TREAT) %>%
  summarize(across(c(denom29, retail_sales29, denom39, retail_sales39, denom33, retail_sales33), function(.x) sum(.x, na.rm=TRUE))) %>%
  mutate(rspc29 = retail_sales29/denom29,
         rspc39 = retail_sales39/denom39,
         rspc33 = retail_sales33/denom33,
...
# for plotting
x <- bystate_long %>% filter(yr == 29) %>% ungroup() %>% rename(rspc2933 = retail_sales_percap_growth_29_33,
                                                                rspc3339 = retail_sales_percap_growth_33_39) %>%
  select(c(AB, TREAT, rspc2933, rspc3339)) %>%
  pivot_longer(rspc2933:rspc3339)

x_avg <- x %>% group_by(name, TREAT) %>% summarize(AB = ifelse(TREAT == 1, "Treated Average", "Control Average"),
                                                   value = mean(value))
